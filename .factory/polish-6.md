...
Candidate: `0.1.15`. Evidence paths are relative to `.factory/`; live checks use `https://food-log-export-kit.sociobot.in` after the matching site deployment.
...
| F-2-5 | Bumped static and rendered footer identifiers together to `0.1.15`. | `keeps the static and rendered footer build identifiers in sync`; live `/not-a-real-route`. |
...
| F-5-2 | Kept the tagged release-workflow claim. | `@claim:release-workflow`; published `v0.1.15` release. |
...
| F-6-1 | Bumped to a new candidate and made its identity check fetch the exact `v0.1.15` release tag rather than mutable latest-release metadata. The tag, release target, `latest.json`, checksums, installer URLs, and deployed identity must all name the tagged checkout commit. | `@claim:candidate-installers`; release `v0.1.15`; live `/release-identity.json`. |
...
| F-6-3 | Regenerated the complete copy audit with the current `0.1.15` build text and README sentences. | `.factory/copy-audit.md`; `evidence/polish-6/landing-desktop-1440.png`; live `/`. |
