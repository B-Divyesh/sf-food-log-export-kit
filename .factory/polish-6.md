...
Candidate: `0.1.16`. Evidence paths are relative to `.factory/`; live checks use `https://food-log-export-kit.sociobot.in` after the matching site deployment.
...
| F-2-5 | Bumped static and rendered footer identifiers together to `0.1.16`. | `keeps the static and rendered footer build identifiers in sync`; live `/not-a-real-route`. |
...
| F-5-2 | Kept the tagged release-workflow claim. | `@claim:release-workflow`; published `v0.1.16` release. |
...
| F-6-1 | Bumped to a fresh candidate after `v0.1.15` targeted the previous commit. Its identity check fetches the exact `v0.1.16` release tag rather than mutable latest-release metadata. The tag, release target, `latest.json`, checksums, installer URLs, and deployed identity must all name the tagged checkout commit. | `@claim:candidate-installers`; release `v0.1.16`; live `/release-identity.json`. |
...
| F-6-3 | Regenerated the complete copy audit with the current `0.1.16` build text and README sentences. | `.factory/copy-audit.md`; `evidence/polish-6/landing-desktop-1440.png`; live `/`. |

## Final verification

The immutable candidate is the commit referenced by annotated tag `v0.1.16`. The GitHub release, peeled tag, `target_commitish`, `latest.json`, `SHA256SUMS`, installer links, and deployed `/release-identity.json` all resolve to that commit.

- `npm test`: 35 unit checks and 58 browser checks pass; four desktop-project skips are the mobile-only checks that pass in the mobile project.
- `npm run build:site`, `npm run build:app`, `npm run native:prereqs`, and `cargo test --manifest-path src-tauri/Cargo.toml` pass.
- All 25 exact commands in `.factory/claims.json` pass independently from a clean clone of `v0.1.16`.
- The `@claim:candidate-installers` test downloads a published installer and verifies its SHA-256 after checking the tag and every release identity field.
- Local evidence screenshots: `evidence/polish-6/landing-desktop-1440.png`, `demo-mobile-390.png`, `app-desktop.png`, and `not-found-desktop.png`.
- Cold production checks cover `https://food-log-export-kit.sociobot.in/`, `/demo`, `/app`, `/privacy`, `/terms`, `/not-a-real-route`, and `/release-identity.json`.
