...
Release candidate `0.1.14` repairs every finding in reviews 1–6. It keeps the local-first Tauri desktop app and the archive-at-dusk visual system. The candidate is tagged and published only from a clean `main` commit; its static site build embeds that same commit in `release-identity.json`.
...
- Bumped the desktop, static-site, Tauri, Cargo, package-lock, and static-404 version together to `0.1.14`.
...
Final evidence is captured from a dependency-clean clone of the annotated `v0.1.14` tag. Every one of the 25 exact commands in `.factory/claims.json` passed independently, including the published-installer download and SHA-256 check. The full suite passed 35 unit tests and 58 browser tests; four desktop-project skips are the mobile-only checks that passed in the mobile project.
...
1. Push the clean `main` candidate and annotated `v0.1.14` tag.
