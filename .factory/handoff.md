...
Release candidate `0.1.13` repairs every finding in reviews 1–6. It keeps the local-first Tauri desktop app and the archive-at-dusk visual system. The candidate is tagged and published only from a clean `main` commit; its static site build embeds that same commit in `release-identity.json`.
...
- Bumped the desktop, static-site, Tauri, Cargo, package-lock, and static-404 version together to `0.1.13`.
...
Run from a clean clone at the tagged commit:
...
Run every command in `.factory/claims.json` independently. In particular, these round-6 checks must pass after the release exists:
...
Browser evidence is recorded under `.factory/evidence/polish-6/`. The cold live audit covers `/`, `/demo`, `/app`, `/privacy`, `/terms`, and an unknown path; it checks titles, headings, route focus, demo isolation, request boundaries, 404, and accessibility.

## Release and deployment

1. Push the clean `main` candidate and annotated `v0.1.13` tag.
