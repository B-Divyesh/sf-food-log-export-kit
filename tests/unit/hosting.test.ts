...
describe('static hosting routes', () => {
  ...
  it('gives the static 404 page the shared metadata, navigation, and footer', () => {
    ...
    for (const marker of ['rel="canonical"', 'property="og:title"', 'property="og:url"', 'name="twitter:card"', 'apple-touch-icon', 'Main navigation', 'href="/terms"', 'Built by Param Factory', 'Version 0.1.12 · release repair']) {
  ...
  it('keeps the static and rendered footer build identifiers in sync', () => {
    ...
    expect(page).toContain(build.replace('${appVersion}', '0.1.12'));
