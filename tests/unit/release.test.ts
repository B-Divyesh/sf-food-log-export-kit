...
describe('release asset selection', () => {
  ...
  it('@regression:V14-release-identity requires both current version and current candidate identity', () => {
    const current = {
      ...
      html_url: 'https://github.com/B-Divyesh/sf-food-log-export-kit/releases/tag/v0.1.13',
      assets
    };
    expect(selectCurrentRelease(current, '0.1.13', 'current-candidate')).toEqual(current);
    expect(selectCurrentRelease({ ...current, tag_name: 'v0.1.11' }, '0.1.13', 'current-candidate')).toBeUndefined();
    expect(selectCurrentRelease({ ...current, target_commitish: 'older-candidate' }, '0.1.13', 'current-candidate')).toBeUndefined();
  });
  ...
  it('@regression:R12-release-absence fetches api.github.com and fails soft for missing, stale, or malformed metadata', async () => {
    ...
    await expect(loadCurrentRelease('0.1.13', 'candidate', { storage, fetcher: missingFetch as typeof fetch, now: 100 }))
    ...
    await expect(loadCurrentRelease('0.1.13', 'candidate', {
      storage,
    ...
    await expect(loadCurrentRelease('0.1.13', 'candidate', {
      storage,
      fetcher: async () => new Response(JSON.stringify({ tag_name: 'v0.1.13', target_commitish: 'candidate' }), { status: 200 })
    })).resolves.toBeUndefined();
  });

  it('@regression:R12-release-cache uses candidate metadata for one hour and refreshes an expired entry', async () => {
    const current = {
      tag_name: 'v0.1.13', target_commitish: 'candidate', html_url: 'https://example.test/current', assets
    };
    ...
    await expect(loadCurrentRelease('0.1.13', 'candidate', { storage, fetcher: fetcher as typeof fetch, now: 100 }))
      .resolves.toEqual(current);
    await expect(loadCurrentRelease('0.1.13', 'candidate', { storage, fetcher: fetcher as typeof fetch, now: 3_600_099 }))
      .resolves.toEqual(current);
    expect(requests).toBe(1);
    await loadCurrentRelease('0.1.13', 'candidate', { storage, fetcher: fetcher as typeof fetch, now: 3_600_100 });
