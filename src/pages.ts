...
export function landingPage(): string {
  ...
  return `${header()}<main id="main">
    <section class="hero">
      ${art}
      <div class="hero-copy"><p class="eyebrow light">Export food tracker history</p><h1 tabindex="-1">Save your food history</h1><p class="hero-lede">For calorie tracker users who need years of meals and recipes in files they control.</p>
      ...
      <div class="hero-caption"><p>Keep a CSV for spreadsheets and JSON for a complete local record.</p></div>
    </section>
    <section class="download-band" aria-labelledby="download-title"><div><p class="eyebrow">Desktop app · version 0.1.2</p><h2 id="download-title">Download the desktop app</h2><p>The app reads CSV and JSON exports. It also works when your internet is off.</p></div><div class="download-action"><a class="primary-button download-button" id="platform-download" href="https://github.com/B-Divyesh/sf-food-log-export-kit/releases">Downloads are being published</a><small id="download-note">Choose a build for your computer from the release page.</small></div></section>
    <section class="product-preview" aria-labelledby="preview-title"><div class="section-intro"><p class="eyebrow">Review before export</p><h2 id="preview-title">See every row before you export</h2><p>Missing or invalid dates and unreadable or comma-formatted numbers appear as notes. Skipped files and rows are named.</p></div>
    ...
    <section class="boundaries" aria-labelledby="boundaries-title"><div><p class="eyebrow light">Privacy and limits</p><h2 id="boundaries-title">How the app handles your files</h2></div><ul><li><b>Your files stay local.</b><span>The app has no food-data server.</span></li><li><b>No account scraping.</b><span>Use exports you requested from your tracker.</span></li><li><b>No nutrition advice.</b><span>Numbers are copied and labeled, not judged.</span></li><li><b>No silent drops.</b><span>Skipped files and rows appear in conversion notes.</span></li></ul></section>
    <section class="pricing" aria-labelledby="price-title"><div class="price-copy"><p class="eyebrow">Batch-import license</p><h2 id="price-title">Combine several exports for $19</h2><p>A free archive handles one file at a time. The license adds multi-file selection for migrations split across years or apps.</p><ul><li>One-time purchase</li><li>Paste the license token on another device</li><li>CSV and JSON export stay free</li></ul></div><div class="price-ticket"><span>PERSONAL LICENSE</span><strong><sup>$</sup>19</strong><small>one time</small><a class="primary-button" href="${checkoutUrl()}" rel="external">Buy the batch license</a><p>Checkout is hosted by Sociobot. <a href="/terms" data-link>Read the terms.</a></p></div></section>
  </main>${footer()}`;
}
...
export function termsPage(): string {
  ...
  return `${header()}<main id="main" class="prose-page"><p class="eyebrow">Effective 28 August 2026</p><h1 tabindex="-1">Terms for using the export kit</h1><p class="lede">Use this tool with files you have the right to access.</p><h2>What the app does</h2><p>The app converts supported fields from CSV and JSON files. Review conversion notes before deleting an old account or original export.</p><h2>Not medical advice</h2><p>The app preserves supplied values. It does not check nutrition accuracy or provide health advice.</p><h2>Batch-import license</h2><p>A $19 personal license adds batch file selection. A refunded or revoked license stops batch access. Core single-file export remains available.</p><h2>Warranty and responsibility</h2><p>The software is provided as is. Keep the original tracker export and verify the result before relying on it. You are responsible for your own backups.</p><h2>Contact</h2><p>Email <a href="mailto:support@sociobot.in">support@sociobot.in</a> for product or purchase questions.</p></main>${footer()}`;
}
...
export async function resolvePlatformDownload(): Promise<void> {
  ...
  try {
    ...
    note.textContent = asset ? `${asset.name} · Check the release notes before installing.` : platform === 'macOS' ? 'Choose Apple Silicon or Intel on the release page.' : 'Downloads are being published for all three platforms.';
  } catch {
